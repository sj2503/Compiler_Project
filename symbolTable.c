...
{ // check for unary expression as well
    if (node == NULL)
        return -1;
    ...
    if (name == L_UNARYEXPR)
    {
        ...
        if (type == L_BOOLEAN)
        {
            printf("SEMANTIC ERROR: Type mismatch in line %d", node->child->TI->lineNo);
        }
        else
            return type;
    }
    ...
    else if (name == L_ID)
    {
        ...
        {
            printf("SEMANTIC ERROR: %s type mismatch in line %d (Expected ID got ARRAY)\n", node->TI->val.lexeme, node->TI->lineNo);
            return ERROR_OCCURED; // SHOULD SAVE SEMANTIC ERROR HERE
        }
        return temp->name;
    }
    else if (name == L_ARRAY_INDEX)
    {
        ...
        {
            // int temp = expressionType(node->child);
    ...
    else if (name == L_ARRAY_ACCESS || name == L_ARRAY_ASSIGN)
    {
        ...
        else if (temp->name == L_ARRAY)
        {
            ...
            int index = expressionType(temp2);
            if (index == L_INTEGER)
            { // if access expression is of L_NUM continue check
...
//         strcpy(node->lexeme, root->TI->val.lexeme);
//         node->lineNo = line;
//         node->isAssigned = false;
//         node->next = NULL;
//         whileEx *temp = whileList;
//         if (temp == NULL)
//         {
//             whileList = node;
//         }
...
//     }
//     if (root->child)
//         constructWhileList(root->child, line);
//     if (root->list_head)
//         constructWhileList(root->list_head, line);
//     if (root->sibling)
//         constructWhileList(root->sibling, line);
// }
// void checkWhileSemantic()
// {
//     whileEx *temp = whileList;
//     int line = temp->lineNo;
//     bool check = false;
//     while (temp)
//     {
//         if (line != temp->lineNo)
...
//             if (check == false)
//             {
//                 printf("SEMANTIC ERROR: none of the Id's in the while expression in lineNO %d is being reassigned inside the while scope", line);
//             }
//             check = false;
//             line = temp->lineNo;
//         }
...
void constructSymbolTable(nodeAST root)
{
        ...
        if (node->name == L_ID)
        {
            ...
            if (isDeclared(node->parent) == true)
            {
                if (!exist)
                {
                    ...
                    {
                        // ERROR
                        printf("Redeclaration of output parameter %s in line %d\n", node->TI->val.lexeme, node->TI->lineNo);
                    }
                    // }
                    else
                    {
                    ...
                    while (temp->name != L_MODULE && temp->name != L_DRIVERMODULE && temp->name != L_MODULEDECLARATIONS)
                    {
                        if (temp->name == L_FOR)
                        {
                            ...
                            {
                                printf("Redeclaration of for loop control variabe %s in line %d.\n", node->TI->val.lexeme, node->TI->lineNo);
                            }
                        }
                        temp = temp->parent;
                    }
            ...
            else
            {
                ...
                {
                    // check in the expression of while
                    // whileEx *temp2 = whileList;
                    // while (temp2)
                    // {
                    //     if (strcmp(temp2->lexeme, node->TI->val.lexeme))
                    //     {
                    //         temp2->isAssigned = true;
                    //     }
                    //     temp2 = temp2->next;
                    // }
                    if (!isPresent)
    ...
    if (node->name == L_WHILE)
    {
        // constructWhileList(node->child, node->child->TI->lineNo);
    }
}

void handleModuleReuse(nodeAST node)
{

    if (node->child == L_MODULEREUSESTATEMENT)
    {
    }
    else
    {
...
void printSymbolTable2(symbolTable *table)
{
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        if (table->localTable[i])
        {
            ...
            hashElem temp = table->localTable[i];
            while (temp != NULL)
            {
                data_ptr *ptr = (data_ptr *)temp->entry;
                if (ptr->var != NULL)
                {
                    // printf("\nPTR->VAR NOT NULL\n");
...
void AST_node_calc(nodeAST root, int *num_ast_nodes, int *mem_ast_nodes)
{
    if (root == NULL)
        return;
    *num_ast_nodes += 1;
    *mem_ast_nodes += sizeof(*root);
    int flag1, flag2, flag3 = 0;
    if (root->child)
    {
        flag1 = 1;
    ...
    {
        // printf("C -- ");
        AST_node_calc(root->child, num_ast_nodes, mem_ast_nodes);
    }
    if (flag2)
    {
        // printf("L -- ");
        AST_node_calc(root->list_head, num_ast_nodes, mem_ast_nodes);
    }
    if (flag3)
    {
        // printf("S -- ");
        AST_node_calc(root->sibling, num_ast_nodes, mem_ast_nodes);
    }
...
void main(int argc, char *argv[])
{
    ...
    int num_ast_nodes = 0;
    int mem_ast_nodes = 0;
    // AST_node_calc(astRoot,&num_ast_nodes,&mem_ast_nodes);
    fclose(file_ptr);
